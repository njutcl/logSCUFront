...
Page({
  ...
  onLoad: function (options) {
    ...
    if (search) {
      ...
      that.loadOrder2([], 1, search, timePage, 1, '');
      that.loadOrder3([], 2, search, pricePage, '', 1);
    }
    else {
      that.loadOrder([], 0, search, normalPage, '', '');
      that.loadOrder2([], 1, search, timePage, 1, '');
  ...
  onNavBarTap: function (event) {
    ...
    if (timePage == 1 && navbarTapIndex == 1) {
      ...
      that.loadOrder2(orderList, 1, '', 1, 1, '');
    }
    if (pricePage == 1 && navbarTapIndex == 2) {
      var orderList = that.data.priceList;
      that.resetOrderList(2);
      that.loadOrder3(orderList, 2, search, 1, '', 1);
    }
  },
  ...
  loadOrder2(orderList, index, search, page, orderByTime, orderByPrice) {
    var that = this;
    wx.showLoading({
      title: '加载中',
      icon: 'loading',
  ...
  loadOrder3(orderList, index, search, page, orderByTime, orderByPrice) {
    var that = this;
    wx.showLoading({
      title: '加载中',
      icon: 'loading',
    ...
    wx.request({
      url: app.globalData.url + '/order3/search',
      data: {
        //Ocampus: 3,
        search: search, //可有可无
        page: page,
    ...
    if (flag == 1) {
      ...
      if (length < LENGTH) { //数据加载完全，但showToast未触发
        ...
        if (index == 1) {
          ...
          that.loadOrder2(orderList, 2, search, pricePage, '', 1);
        } else if (index == 0) {
          that.loadOrder3(orderList, 0, search, normalPage, '', '');
        }
      }
    }
  },
  ...
  getOrderInfo(e) { //获得某个订单的更多信息，需要登录
    ...
    var weight = orderInfo.goods_weight;
    var goods_category = orderInfo.goods_category;
    var hidden_info = orderInfo.hidden_info;
    ...
    wx.request({
      ...
      success(res) {
        ...
        wx.navigateTo({
          url: '../recommendation/daiqu/daiqu?order=' + order + '&received_pos=' + received_pos,
        })
      }
    })
  },
  ...
  getOrderInfo2(e) { //获得某个订单的更多信息，需要登录
    var that = this;
    //console.log(e.currentTarget.dataset.index);
    var index = e.currentTarget.dataset.index;
    var orderList = that.getOrderList(that.data.navbarActiveIndex);
    ...
    wx.request({
      ...
      success(res) {
        ...
        wx.navigateTo({
          url: '../recommendation/paotui/paotui?order=' + order + '&received_pos=' + received_pos,
        })
      }
    })
  },
  ...
  getOrderInfo3(e) { //获得某个订单的更多信息，需要登录
    var that = this;
    //console.log(e.currentTarget.dataset.index);
    var index = e.currentTarget.dataset.index;
    var orderList = that.getOrderList(that.data.navbarActiveIndex);
    ...
    wx.request({
      ...
      success(res) {
        ...
        wx.navigateTo({
          url: '../recommendation/second_hand/second_hand?order=' + order + '&received_pos=' + received_pos,
        })
      }
    })
  },
