...
Page({
  ...
  ownerdetai: function (options) {
    ...
    wx.navigateTo({
      url: '/news/pages/recommendation/ownerdetai/ownerdetai?head=' + order.order_owner.head_img + '&name=' + order.order_owner.wx_name
    })
  },
  ...
  handleOrder(e) {
    ...
    order.weight = (parseFloat(order.money)).toFixed(2);
  ...
  receiveOrder() {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          wx.request({
            url: app.globalData.url + '/order2/receiveOrder',
            data: {
              //sessionid: wx.getStorageSync('sessionid'),
